...
void main()
{
	...
	//	daColor = diffColour;
	//else
	//	daColor = vec4(diffColour.x + 0.5f, diffColour.y, diffColour.z + 0.5f, diffColour.a);
	//return;

	//diffuseTexture = texture(diffTexture, fTexUV.xy);
	//normalTexture = normalize(fNormal);
	//positionTexture = fVtxWorldPos;

	FragColour[0] = texture(diffTexture, fTexUV.xy);
	FragColour[1] = fNormal;
	FragColour[2] = fVtxWorldPos;

	//daColor = texture(diffTexture, fTexUV.xy);
